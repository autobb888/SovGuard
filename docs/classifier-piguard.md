...
Runtime writes `tokenizer.compat.json` (Metaspace `prepend_scheme`/`split` downgraded for the Node `tokenizers` crate). Optional HF safetensors parity spot-check is post-merge, not a gate.
