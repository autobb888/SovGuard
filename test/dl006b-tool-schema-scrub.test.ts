...
describe('DL-006b tool-schema scrub→FP→scrub', () => {
  ...
  it('ZW-split and fullwidth tool_call neutralized in schema docs', async () => {
    const schema: ToolSchema = {
      name: 'add',
  ...
  it('benign schema still allows', async () => {
    const res = await scanToolSchema({
      name: 'add',
